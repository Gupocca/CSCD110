...
class Player:
    ...
    ##  tmpList - list; contains keys for the dictionary
        ...
        try:
            ...
            for i in range(4):      self.val[tmpList[i]] = str(vals[i]).strip() # strings
            for i in range(4, 21):  self.val[tmpList[i]] = int(vals[i])         # integers
...
##  cmd - string; command intput from user
    ...
    try: stats = loadStats('player_career.txt') # load the file
    except IOError as e: print(e); sys.exit(0)  # known error
    except: print('An unknown error occured.'); sys.exit(0) # unknown error
...
##  result - string; the accumulator for the resulting data
    ...
    # add each of the top 50 to the list in the format "num. fname, lname \n"
